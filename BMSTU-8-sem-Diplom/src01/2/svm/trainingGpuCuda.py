...
Corpus = pd.read_csv(
    r"C:\Text_Class_SVM\Text-Classification-SVM\BBCNewsTrain2.csv",
    encoding="latin-1",
)
...
# X_train_cudf = cudf.DataFrame(Train_X)
# X_test_cudf = cudf.DataFrame(Test_X)
# y_train_cudf = cudf.Series(Train_Y)
# y_test_cudf = cudf.Series(Test_Y)
...
sns.countplot(x='label', data=Corpus, hue='label', palette='cool', legend=False)
plt.xlabel(' ')
plt.ylabel('Count')
